...
def _build_args(x_in__, weight_in__, y_out_):
    __inputs__ = []
    for arg in [x_in__, weight_in__]:
        if arg != None:
...
@tbe_register.register_operator("TalkerGemv", trans_bool_to_s8=False)
...
def talker_gemv(x_in__, weight_in__, y_out_, kernel_name="talker_gemv", impl_mode = ""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, weight_in__, y_out_)
    ...
    origin_func_name = "talker_gemv"
    ascendc_src_dir_ex = "talker_gemv"
    ascendc_src_dir = "talker_gemv"

    src_file_dict = {'ascend910_93': './op_kernel//talker_gemv.cpp'}
    src_file = get_src_file_from_dict(src_file_dict, soc_short)
    if src_file != "":
        ascendc_src_file = src_file
    else:
        ascendc_src_file = "talker_gemv.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Ascend C Operator TalkerGemv, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "TalkerGemv"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__ , impl_mode = impl_mode, origin_inputs=[x_in__, weight_in__], origin_outputs = [y_out_],\
...
def op_select_format(x_in__, weight_in__, y_out_, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, weight_in__, y_out_)
    result = check_op_cap("op_select_format", "TalkerGemv", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, weight_in__, y_out_, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, weight_in__, y_out_)
    result = check_op_cap("get_op_specific_info", "TalkerGemv", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
