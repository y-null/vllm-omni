...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['logits', 'noise', 'history', 'history_len', 'step', 'finished', 'max_tokens']
    output_names = ['sampled', 'next_history', 'next_history_len', 'next_step', 'finished_out', 'emit']
...
def _build_args(logits_in__, noise_in__, history_in__, history_len_in__, step_in__, finished_in__, max_tokens_in__, sampled_out_, next_history_out_, next_history_len_out_, next_step_out_, finished_out_out_, emit_out_, eos_token_id):
    __inputs__ = []
    for arg in [logits_in__, noise_in__, history_in__, history_len_in__, step_in__, finished_in__, max_tokens_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [sampled_out_, next_history_out_, next_history_len_out_, next_step_out_, finished_out_out_, emit_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
...
@tbe_register.register_operator("TalkerCodecSampleAdvance", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def talker_codec_sample_advance(logits_in__, noise_in__, history_in__, history_len_in__, step_in__, finished_in__, max_tokens_in__, sampled_out_, next_history_out_, next_history_len_out_, next_step_out_, finished_out_out_, emit_out_, eos_token_id, kernel_name="talker_codec_sample_advance", impl_mode = ""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(logits_in__, noise_in__, history_in__, history_len_in__, step_in__, finished_in__, max_tokens_in__, sampled_out_, next_history_out_, next_history_len_out_, next_step_out_, finished_out_out_, emit_out_, eos_token_id)
    ...
    origin_func_name = "talker_codec_sample_advance"
    ascendc_src_dir_ex = "talker_codec_sample_advance"
    ascendc_src_dir = "talker_codec_sample_advance"

    src_file_dict = {'ascend910_93': './op_kernel//talker_codec_sample_advance.cpp'}
    src_file = get_src_file_from_dict(src_file_dict, soc_short)
    if src_file != "":
        ascendc_src_file = src_file
    else:
        ascendc_src_file = "talker_codec_sample_advance.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Ascend C Operator TalkerCodecSampleAdvance, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "TalkerCodecSampleAdvance"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__ , impl_mode = impl_mode, origin_inputs=[logits_in__, noise_in__, history_in__, history_len_in__, step_in__, finished_in__, max_tokens_in__], origin_outputs = [sampled_out_, next_history_out_, next_history_len_out_, next_step_out_, finished_out_out_, emit_out_],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'required', 'required', 'required', 'required', 'required', 'required', 'required', 'required', 'required', 'required', 'required', 'required'], init_value_list = [None, None, None, None, None, None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}', {'valueDepend': {}})

def op_select_format(logits_in__, noise_in__, history_in__, history_len_in__, step_in__, finished_in__, max_tokens_in__, sampled_out_, next_history_out_, next_history_len_out_, next_step_out_, finished_out_out_, emit_out_, eos_token_id, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(logits_in__, noise_in__, history_in__, history_len_in__, step_in__, finished_in__, max_tokens_in__, sampled_out_, next_history_out_, next_history_len_out_, next_step_out_, finished_out_out_, emit_out_, eos_token_id)
    result = check_op_cap("op_select_format", "TalkerCodecSampleAdvance", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(logits_in__, noise_in__, history_in__, history_len_in__, step_in__, finished_in__, max_tokens_in__, sampled_out_, next_history_out_, next_history_len_out_, next_step_out_, finished_out_out_, emit_out_, eos_token_id, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(logits_in__, noise_in__, history_in__, history_len_in__, step_in__, finished_in__, max_tokens_in__, sampled_out_, next_history_out_, next_history_len_out_, next_step_out_, finished_out_out_, emit_out_, eos_token_id)
    result = check_op_cap("get_op_specific_info", "TalkerCodecSampleAdvance", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
